...
>
> 基本機能を確認できるPortfolio MVPですが、画像素材と検証方法は見直し中です。本プロジェクトは、公式データを使用していない学習用プロトタイプです。実在する企業・商品・キャラクター・公式画像・公式デザイン・実際の金型データは使用していません。

## 開発状況と画像素材の方針

現在のランナー画像・完成後画像は、機能説明用に作成した仮素材です。適切な実画像認識データや実用性の根拠としては扱いません。

今後、権利処理済みで撮影条件と用途を説明できるランナー写真・試作サンプル写真を用意する予定です。ただし、継続的な素材準備が難しい可能性もあるため、実物写真に依存せず、架空の模式図、ワイヤーフレーム、検査点マップ、部品特徴データ表で評価フローを説明する別案も検討しています。現時点では最終的な差し替え方法を決定していません。

画像方針の判断基準と差し替え条件は、[画像素材の更新計画](docs/image_asset_plan.md) にまとめています。

## 公開デモとローカル検証の切り分け

GitHub、README、公開Streamlitデモ、同梱スクリーンショットでは、架空ランナー画像と架空データだけを使います。公開デモでは画像アップロードとOllama連携を既定で無効にしています。

一方で、自分のPCで起動したローカル環境では、環境変数で明示的に有効化したうえで、権利処理済み画像、自分で撮影した検証用写真、試作サンプル写真などをアップロードできます。公開しない画像は `local_inputs/` または `private_assets/` に置いてください。これらのディレクトリ内の画像ファイルはGit管理対象外です。
...
画像をアップロードすると、その画像データは起動中のStreamlitプロセスへ送信されます。公開サーバーには機密画像を送らないでください。ローカル実画像で検証する場合も、アプリは画像ファイル名や画像本体をMarkdownレポートに保存しません。保存レポート上では `local_uploaded_image` として扱います。

ローカル検証では、ライセンス確認済み画像や自前写真を想定した入力で、ゲート跡、小型部品、切り出し時の処理負担が評価結果に反映されることを確認しています。ただし、これらの実画像や画像名は公開リポジトリ、README、公開デモ、公開スクリーンショットには含めません。
...
検査画像は、架空ランナー模式図としてアプリ内で生成しています。現在のランナー画像は最終成果の画像としてREADMEには掲載せず、[画像素材の更新計画](docs/image_asset_plan.md) に沿って説明方法を見直しています。

| 部品改善カルテ | ローカルAI補強分析 |
| --- | --- |
| ![Detail carte](docs/screenshots/detail_carte.png) | ![Local LLM panel](docs/screenshots/local_llm_panel.png) |

> 現在の画像はデモ説明用の仮素材です。今後、権利処理済みのランナー写真や試作サンプル写真を用意する予定ですが、準備が難しい場合に備えて実物写真を使わない別案も検討しています。現MVPでは学習済み画像AIではなく、軽量な画像ヒューリスティックで特徴候補を自動提案し、人が確認・修正してルールベース評価に流します。
...
詳細分析画面では、ローカル環境で明示的に有効化した場合に限り、Ollamaによる補強分析を実行できます。接続先は同一PCの `http://localhost:11434/api/generate` に固定し、画面からの変更や外部URLへの接続は拒否します。
...
$env:PLAMO_ENABLE_LOCAL_LLM="true"
streamlit run app.py
```

必要に応じてモデルとタイムアウトを環境変数で変更できます。
...
- ランナー検査ビューでは、説明用の仮素材 `assets/runner_bg_realistic.png` を背景に使い、架空ランナー上へリスクマーカーを重ねています。この画像は検証データや認識精度の根拠には使いません。
- `image_recognizer.py` で、アップロード画像のエッジ密度、小領域候補、中央/外周分布を読み取り、部品サイズ、ゲート位置、材料候補、観察タグを自動提案します。
- シール、説明書、箱、透明袋の反射などが写り込む画像は解析品質注意として警告し、候補枠をそのまま設計判断に使わない導線にしています。
- ローカル画像は自動ROI候補を初期値として提示し、シール、背景、箱、説明書を避けて対象ランナー部分だけを評価できます。
- ランナー画像・特徴データ入力評価では、自動提案された部品サイズ、ゲート位置、材料、切り出し時の負荷、観察特徴を人が確認・修正し、既存のリスクスコアリングへ流し込みます。
- `image_based_analyzer.py` で、生成済みランナー画像に含まれる部品番号、ゲート点、リスク色、検査ラベルを再整理し、画像を見ながら説明できる分析コメントを生成します。
...
アプリ上で画像をアップロードすると、画像内のエッジ分布から自動ROI候補を提示し、その範囲をスライダーで微調整できます。対象ランナー部分だけを切り出して軽量CVへ渡し、対象部位、部品サイズ、ゲート位置、材料、切り出し時の負荷、読み取れた特徴を候補として出します。候補を確認・修正すると、リスクスコア、判定理由、具体的な変更案、推定原因が返ります。

検証に向く画像は、単体ランナーを真上から撮り、シール、説明書、箱、ロゴ、透明袋の強い反射が重なっていないものです。これらが写り込むと、軽量CVは印刷物や反射のエッジをゲート候補として誤検出する可能性があります。アプリはその可能性がある場合に「解析品質注意」を表示します。

入力できる画像はPNG、JPEG、WebP形式、10MB以下、2,000万画素以下です。形式を偽装したファイル、破損画像、展開時に過大になる画像は解析前に拒否します。

`outputs/local_runner_evaluation.md` に保存できるのは評価結果だけです。実画像のファイル名、画像本体、元パス、ROI画像は保存しません。

ローカル検証済みの観点:

- ゲート点が多い画像では、処理負担とゲート跡リスクが評価に反映される
- 小型部品や透明パーツ相当の入力では、紛失・白化・跡の目立ちやすさを確認観点として扱える
- 自動ROI候補により、画像全体ではなくランナーらしい連続エッジ領域を初期解析対象にできる
- 保存レポートには `local_uploaded_image` のみを残し、実画像名や元パスは残さない
...
公式データや外部LLM APIは使っていないため、追加の環境変数なしで起動できます。公開環境では `PLAMO_ENABLE_IMAGE_UPLOAD` と `PLAMO_ENABLE_LOCAL_LLM` を設定せず、安全側の既定値を維持してください。

## セキュリティ方針

- 公開版では画像アップロードとOllama通信を既定で無効にする
- Ollama接続先を同一PCの標準ポートに固定し、任意URLへのサーバー側通信を拒否する
- アップロード画像の形式、容量、総画素数を解析前に検証する
- `.env`、`.streamlit/secrets.toml`、実画像、ローカル評価結果をGit管理対象外にする
- Python依存関係を固定し、CIで脆弱性監査を行う
- GitHub Actionsは読み取り権限だけを付与し、利用するActionをコミットSHAで固定する
...
- ランナー画像・特徴データ入力評価: ランナー写真や模式図を入力し、軽量CVが部品サイズ、ゲート位置、切り出し時の負荷などを自動提案します。提案を確認・修正すると、リスクスコア、原因候補、具体的な変更案が返ります。
...
- 第一案として、権利処理済みのランナー写真や試作サンプル写真を用意し、撮影条件と注釈基準をそろえて画像解析を検証する
- 素材準備が難しい場合は、模式図、ワイヤーフレーム、検査点マップ、部品特徴データ表だけで評価フローを説明できる構成へ切り替える
- 画像方針が決まるまでは、現在の仮画像を実画像認識の精度や実用性の根拠として扱わない
